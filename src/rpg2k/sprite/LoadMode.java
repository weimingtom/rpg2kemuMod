...
public class LoadMode extends Mode
{
	...
	public void paint(Graphics2D g2) {
		...
		for(i = 0; i < 3; i++)
			g2.drawImage(FONT.getString(header+decSpace(OFFSET+i+1, 2),
				GAME_DATA.SAVE_DATA.exists(i) ? FNT_ENABLE : FNT_DISABLE),
				8, 50+i*64, null);
	}
	...
	public void processKey(GameKey key) {
		switch(key) {
			case ENTER:
				if(GAME_DATA.SAVE_DATA.exists(SELECT)) {
					owner.setSaveData(SELECT);
